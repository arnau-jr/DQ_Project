...
plt.colorbar(cont_2d,label=r"$|\Psi(r,R)|$")  # Plots the colorbar for 2d contour diagram

# Loop over both axes to label them and adjust the tick sizes
for a in fig.axes[:-1]:
    a.set_ylabel(r'$R(\rm a_0)$', size=10)
    a.set_xlabel(r'$r(\rm a_0)$', size=10)
    a.tick_params(labelsize=5)

ax_3d.set_zlabel(r"$|\Psi(r,R)|$", size = 10)
